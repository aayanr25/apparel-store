  ...
  async fetch(request, env) {
    ...
    if (url.pathname === '/api/image') {
      ...
      if (fileObj.type === 'file' && fileObj.file?.url) {
        // Standard Notion-hosted file — URL is already signed and fresh
        freshUrl = fileObj.file.url;
      } else if (fileObj.type === 'external' && fileObj.external?.url) {
        // External URL (rare but handle it)
        freshUrl = fileObj.external.url;
      }

      // Attachment-style files have a file:// URI — retrieve signed URL via Notion API
      if (!freshUrl) {
      ...
      // Proxy the image bytes back to the browser
